...
#set the PAT
def get_user_pat():
    pat = st.sidebar.text_input("Enter your Personal Access Token (PAT):", type="password")
    return pat
...
### analyse text ---[generate]---> {resume} : {score}
def analyze_resume(resumetext,job_description):
    try:
        ...
        analyse the candidate's resume against the job description by looking for these following qualities:
          ...
          6. Check projects title, technology used is match with jobs technology or not
          7. Education - The candidate's educational background
          8. Educational quality
          9. Certifications: specialized training, especially if they align with the job requirements.
          10. Technical skills
        Step-3:
...
# Streamlit interface
st.title("JobFit Evaluator")

# Get PAT from the user
pat = get_user_pat()

if not pat:
    st.warning("Please enter your Personal Access Token (PAT) in the sidebar.")
    st.stop()  # Stop further execution if PAT is not provided

    # Continue with the rest of your app logic here
st.success("You have successfully entered your PAT!")
st.write(f"Your PAT: {pat}")
...
llm = Clarifai(pat=pat, user_id='clarifai', app_id='ml', model_id='llama2-70b-chat-alternative')
template = """
Temperature: {temp}
top_k: {top_k}
top_p: {top_p}
...
if st.button("Submit"):
    if resumes:
        ...
        if len(scores_df) >= 3: 
            ...
            top_3_resumes = scores_df.sort_values(by="score", ascending=False)
